...
import { ArrowLeft, UserPlus, User, Mail, Lock, BookOpen, Phone, MapPin, Users2 } from 'lucide-react';
...
const AdminStudentAdd = () => {
  ...
  const [form, setForm] = useState({
    name: '', email: '', password: '', course: '', phone: '', address: '', guardianName: '', guardianPhone: '', fatherName: '', motherName: '', batch: ''
  });
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-6 max-w-3xl">
      ...
      <div className="flex items-center gap-2 text-xs font-bold text-gray-400 uppercase tracking-widest">
        ...
        <span>›</span>
        <span className="text-primary-600">Add New Student</span>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm font-medium">Fill in the student details below.</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-[24px] p-8 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input type="text" name="name" required value={form.name} onChange={handleChange} className={inputClass} placeholder="John Doe" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-2">Email Address *</label>
              <input type="email" name="email" required value={form.email} onChange={handleChange} className={inputClass} placeholder="john@school.edu.in" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-2">Password *</label>
              <input type="text" name="password" required value={form.password} onChange={handleChange} className={inputClass} placeholder="Temporary password" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-2">Phone Number</label>
              <input type="text" name="phone" value={form.phone} onChange={handleChange} className={inputClass} placeholder="+91 9876543210" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[24px] p-8 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input type="text" name="course" value={form.course} onChange={handleChange} className={inputClass} placeholder="Computer Science" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-2">Batch / Year</label>
              <input type="text" name="batch" value={form.batch} onChange={handleChange} className={inputClass} placeholder="2026" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[24px] p-8 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input type="text" name="guardianName" value={form.guardianName} onChange={handleChange} className={inputClass} placeholder="Parent / Guardian" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-2">Guardian Phone</label>
              <input type="text" name="guardianPhone" value={form.guardianPhone} onChange={handleChange} className={inputClass} placeholder="+91 9876543210" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-2">Father's Name</label>
              <input type="text" name="fatherName" value={form.fatherName} onChange={handleChange} className={inputClass} placeholder="Father's full name" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-2">Mother's Name</label>
              <input type="text" name="motherName" value={form.motherName} onChange={handleChange} className={inputClass} placeholder="Mother's full name" />
            </div>
            <div className="md:col-span-2">
              <label className="block text-xs font-bold text-gray-500 mb-2">Address</label>
              <input type="text" name="address" value={form.address} onChange={handleChange} className={inputClass} placeholder="Full residential address" />
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          >
            <UserPlus className="w-4 h-4" /> {loading ? 'Creating...' : 'Create Student'}
          </motion.button>
        </div>
      </form>
    </motion.div>
